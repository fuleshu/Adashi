...
The `adashi_rules` tool's `get_rule_injections` operation still accepts projectId, intend and hook. Its canonical executable body remains `injectionPrompt`. Apply it once even if `rules` is empty. An empty hook explicitly returns `status: "empty"`, an empty prompt, and empty section/rule arrays. No lifecycle hooks have been removed.
