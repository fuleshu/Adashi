...
Project-specific memory instructions, when configured, are supplied in full as a separate section. The built-in default is empty: shared Adashi workflow belongs in `agents_template.md`, and operation syntax is retrieved through `adashi_help`. Default memoryContext=summary supplies at most 2,000 Unicode characters of current summary, never historical handovers. An oversized summary is omitted in full with an explicit omission notice; the template instructs agents to retrieve it before work needing project constraints. memoryContext=protocolOnly omits that summary section for operational requests. General intent does not imply that memory is irrelevant.
...
Design and implementation startup include any configured project-specific fixed guidance and a metadata-only index, bounded to 3,000 UTF-8 bytes. Generic fixed-hook defaults and the built-in design-write protocol are no longer injected. Known old defaults, including the full authoring prompt containing expectedRevision and its repaired tool-name variants, migrate once to empty; custom text survives and can be explicitly cleared. The SQL projection reads ids, names, parent ids, element types and versions for at most 32 entries, and reports included/total counts. Oversized rows are omitted whole; retrieval ids are never truncated. One explanation covers omitted descriptions, relationships, bindings, artifacts and source.
