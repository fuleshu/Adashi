...
const ROOT_DESCRIPTION_CHARS: usize = 170;
...
const ROOT_SUMMARY_PLACEHOLDER: &str =
    "Top layer: 00 of 000 elements, 00 of 0000 relationships. Deeper components, artifacts and \
     file bindings are retrievable on demand with the adashi_design get_scope and get_bindings \
     operations.";
...
fn generated_notice() -> &'static str {
    "Generated by Adashi from the formal design model. Do not edit this block: change the \
     design in Adashi and the block is regenerated."
}
...
fn render_root_block(snapshot: &DesignSnapshot, revision: i64) -> String {
    ...
    // The body is fitted first so the summary line can state what actually survived.
    let mut body = element_lines;
    let dropped_elements = fit_budget(&mut body, budget);
    ...
    block[ROOT_SUMMARY_INDEX] = format!(
        "Top layer: {top_layer_count} of {total_elements} elements, {} of {total_relationships} \
         relationships. Deeper components, artifacts and file bindings are retrievable on \
         demand with the adashi_design get_scope and get_bindings operations.",
...
fn render_folder_block(
    ...
    let header = vec![
        ...
        "Generated from the Adashi design model; do not edit, change the model.".to_string(),
